...
def aggregate(args):
    ...
    if template_grp:
        for arr, guide_arr in zip(tss_arrays, guide_tss):
            new_arr = _group_tss(arr, before=region, after=region, template_tss_arr=guide_arr, set_to_zero=True)
...
def main():
    ...
    parent_parser.add_argument("-t", "--toInt", required=False, action="store_true", default=True,
    ...
    parser_aggregate.add_argument('-s', '--set_to_zero', action='store_true', default=True,
                                  help='set tss to zero where positions were zero in template, default=True')
